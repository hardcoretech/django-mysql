...
def modify_sql_index_hints(sql, table_name, rule, index_names, for_what, start_pos):
    ...
    last_match_start = matches[len(matches) - 1].start(0)

    return sql[0:last_match_start] + re.sub(table_spec_re, replacement, sql[last_match_start:], count=1, flags=re.VERBOSE)
